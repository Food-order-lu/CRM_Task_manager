...
document.addEventListener('DOMContentLoaded', () => {
    // Buttons are in static HTML now
    ...
    // Toggle for new commerce in modal
            ...
            return `
                <tr class="hover:bg-white/5 transition-all group">
                    ...
                    <td class="p-4 hidden md:table-cell">
                        <span class="text-[10px] bg-white/5 px-2 py-1 rounded text-gray-400 border border-white/5 whitespace-nowrap">${task.category.replace(/^[^\s]+\s/, '')}</span>
                    </td>
                    <td class="p-4">
                    ...
                    <td class="p-4 text-center">
                        <div class="flex items-center justify-center space-x-2 opacity-0 group-hover:opacity-100 transition-all">
                            ...
                            ` : ''}
                            <button class="text-gray-400 hover:text-red-400" onclick="window.deleteTask('${task.id}', '${task.name}')">✕</button>
                        </div>
                    </td>
                </tr>
            `;
                        ...
                        ${columnTasks.map(t => `
                            <div class="glass-card p-4 hover:border-blue-500/30 transition-all cursor-move relative group" draggable="true" ondragstart="window.dragTask(event, '${t.id}')">
                                ...
                                <div class="flex justify-between items-center text-[10px] text-gray-400">
                                    <span class="bg-white/5 px-2 py-0.5 rounded border border-white/5">${t.category.split(' ')[1] || t.category}</span>
...
window.cycleTaskStatus = (id, currentStatus) => {
    ...
    updateTaskStatus(id, sequence[currentStatus] || 'To do');
};
...
document.getElementById('task-form')?.addEventListener('submit', async (e) => {
    ...
    const assignees = Array.from(form.querySelectorAll('input[name="assignees"]:checked')).map(cb => cb.value);
